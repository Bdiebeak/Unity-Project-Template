...
- [ ] TODO
Но все же я решил подготовить такой код. Скоро он появится здесь.
...
- **Ведение репозитория**</br>
Ведение репозитория должно быть прозрачным, понятным, разделенным на конкретные стадии разработки.
Принято выделять несколько основных веток при ведении репозитория:
1. **Main** - главная ветка проекта. В каких-то проектах пустая, в каких-то ее версия соответствует последней версии билда - решать вам. Настроено релизное окружение;
2. **Develop** - основная ветка разработки. Законченные задачи добавляются в нее. Настроено тестовое окружение;
Я бы добавил еще несколько веток для разработки в Unity:
1. **Production** - ветка, включающая в себя готовые задачи для следующего релиза. Настроено релизное окружение;
2. **Art** - отдельная ветка для работы графических дизайнеров и не программистов;
3. **Features** - это скорее совокупность веток. В ней разработчики создают новую ветку под каждую новую задачу. Если в проекте используется какая-нибудь система таск-трекинга,  название ветки может включать в себя номер задачи. Например: `features/11-player-movement`.

В добавку про нейминг:
1. Имена должны четко говорить о том, что было изменено/добавлено в данном коммите; 
2. Именуя коммит, нужно следовать договоренностям внутри команды. Но наиболее часто имя коммита использует настоящее время в повелительном наклонении;
...
- **Регулярный профайлинг и тестирование**</br>
...
1) **Раздельные** - отключает конкретный функционал. Например, наличие `LOGS` в параметрах включает логи, а его отсутствие - отключает;
2) **Комбинированные** - отключает совокупность функционала. Например, наличие `PRODUCTION` отключает весь дебажный функционал;
